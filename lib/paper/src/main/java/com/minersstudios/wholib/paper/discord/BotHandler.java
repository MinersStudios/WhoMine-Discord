package com.minersstudios.wholib.paper.discord;
...
import static com.minersstudios.wholib.utility.ChatUtils.serializePlainComponent;
...
public final class BotHandler {
    ...
    private void handleSkinTask(final @NotNull Skin skin) {
        ...
        this.waitingReplyTask = () -> {
            ...
            switch (actionIndex) {
                ...
                case 3 -> this.plugin.runTask(() -> removeSkin(this.playerInfo, skin, message));
                default -> {
                    this.reply(DISCORD_SKIN_INVALID_INDEX.asString());
    ...
    public static boolean editSkin(
            final @NotNull PlayerInfo playerInfo,
        ...
        if (player != null) {
            MSLogger.fine(
                    player,
                    DISCORD_SKIN_SUCCESSFULLY_EDITED_MINECRAFT.asTranslatable().arguments(text(skinName))
            );
        }
