...
import com.minersstudios.whomine.custom.block.event.CustomBlockPlaceEvent;
...
import com.minersstudios.whomine.custom.block.event.CustomBlockBreakEvent;
import com.minersstudios.whomine.utility.BlockUtils;
import com.minersstudios.whomine.utility.CoreProtectUtils;
 ...
 *
 * @see CustomBlockData
 * @see #place(WhoMine, Player, EquipmentSlot, BlockFace, Axis)
 * @see #destroy(WhoMine, Player)
 */
public final class CustomBlock {
    ...
    public void place(
        ...
        plugin.runTask(() -> {
            ...
            switch (placingType) {
                case final PlacingType.Default normal -> noteBlock = normal.getNoteBlockData().craftNoteBlock(blockData);
                case final PlacingType.Directional directional -> {
                    if (blockFace == null) {
                        throw new IllegalArgumentException(
                                "Block face is null, but placing type is directional! " + key
                        );
                    }
                ...
                default -> throw new IllegalArgumentException(
                        "Unknown placing type: " + placingType.getClass().getName()
                );
            }
