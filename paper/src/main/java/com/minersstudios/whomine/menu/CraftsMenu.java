...
public final class CraftsMenu {
    ...
    static {
        final InventoryButton blockButton = new InventoryButton()
                .clickAction((event, i) -> {
        ...
        final InventoryButton decorButton = new InventoryButton()
                .clickAction((event, i) -> {
                    final Player player = (Player) event.getWhoClicked();
        ...
        final InventoryButton itemButton = new InventoryButton()
                .clickAction((event, i) -> {
                    final Player player = (Player) event.getWhoClicked();
        ...
        CATEGORIES_INVENTORY =
                ...
                .buttons(
                        ...
                        .boxed()
                        .collect(Collectors.toMap(Function.identity(), slot -> blockButton))
                )
                .buttons(
                        IntStream.of(3, 4, 5, 12, 13, 14, 21, 22, 23, 30, 31, 32)
                        .boxed()
                        .collect(Collectors.toMap(Function.identity(), slot -> decorButton))
                )
                .buttons(
                        IntStream.of(6, 7, 8, 15, 16, 17, 24, 25, 26, 33, 34, 35)
                        .boxed()
                        .collect(Collectors.toMap(Function.identity(), slot -> itemButton))
                );
