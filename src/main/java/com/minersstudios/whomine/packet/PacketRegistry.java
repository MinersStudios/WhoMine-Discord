...
import static net.minecraft.network.protocol.PacketFlow.*;
...
public final class PacketRegistry {
    ...
    static {
        ...
        for (final var protocol : protocols) {
            ...
            if (serverMaps.containsKey(protocol)) {
                putPackets(serverMaps.get(protocol), packetProtocol, SERVERBOUND);
            }

            if (clientMaps.containsKey(protocol)) {
                putPackets(clientMaps.get(protocol), packetProtocol, CLIENTBOUND);
            }
        }
    }
    ...
    public static @Nullable PacketType getTypeFromClass(final @NotNull Class<?> packet) {
        return packet == ClientboundBundlePacket.class
                ? PacketType.PLAY_CLIENT_BUNDLE_DELIMITER
